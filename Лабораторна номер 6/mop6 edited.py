...
matrix_with_min_max_x = np.array([[-10, 50], [-20, 60], [-20, 5]])
m = 3
...
while flag_of_model is False:
    ...
    while flag_of_dispersion is False:
        Y_matrix = np.array(
            [8.4 + 8.5 * plan_matr_for_calc_Y[:, 1] + 5.7 * plan_matr_for_calc_Y[:, 2] + 9.7 * plan_matr_for_calc_Y[:, 3] + 8.9 * plan_matr_for_calc_Y[:, 1] ** 2 +
             0.2 * plan_matr_for_calc_Y[:, 2] ** 2 + 0.5 * plan_matr_for_calc_Y[:, 3] ** 2 + 2 * plan_matr_for_calc_Y[:, 1] * plan_matr_for_calc_Y[:, 2] +
             0.7 * plan_matr_for_calc_Y[:, 1] * plan_matr_for_calc_Y[:, 3] + 4.3 * plan_matr_for_calc_Y[:, 2] * plan_matr_for_calc_Y[:, 3] +
             9.7 * plan_matr_for_calc_Y[:, 1] * plan_matr_for_calc_Y[:, 2] * plan_matr_for_calc_Y[:, 3] + random.randint(0, 100) - 50 for i in range(m)]).T
    ...
    if phisher_criterion(Y_matrix, np.size(indexes), N):
        flag_of_model = True
        if len(students_t_test(norm_matrix, Y_matrix, N)) != 2:
